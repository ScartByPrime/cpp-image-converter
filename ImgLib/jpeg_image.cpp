    ...
    bool SaveJPEG(const Path& file, const Image& image) {
        ...
        */
        ...
        while (cinfo.next_scanline < cinfo.image_height) {
             ...
             */
            const Color* src = image.GetLine(cinfo.next_scanline);

            for (int x = 0; x < image.GetWidth(); ++x) {
                scanline[x * 3 + 0] = static_cast<unsigned char>(src[x].r);
                scanline[x * 3 + 1] = static_cast<unsigned char>(src[x].g);
                scanline[x * 3 + 2] = static_cast<unsigned char>(src[x].b);
            }
